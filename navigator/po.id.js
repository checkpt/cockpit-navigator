cockpit.locale({
 "": {
  "plural-forms": (n) => (n != 1),
  "language": "id",
  "language-direction": "ltr"
 },
 "Uploading ": [
  null,
  "Mengunggah "
 ],
 "Failed to read file: ": [
  null,
  "Gagal membaca berkas: "
 ],
 "Upload of directories not supported.": [
  null,
  "Pengunggahan direktori tidak didukung."
 ],
 "Upload of directories and empty files not supported.": [
  null,
  "Pengunggahan direktori dan berkas kosong tidak didukung."
 ],
 "OK": [
  null,
  "OK"
 ],
 "Cancel": [
  null,
  "Batal"
 ],
 "Yes": [
  null,
  "Ya"
 ],
 "No": [
  null,
  "Tidak"
 ],
 "Cannot rename system-critical paths.": [
  null,
  "Tidak dapat mengganti nama jalur kritis sistem."
 ],
 "If you think you need to, use the terminal.": [
  null,
  "Jika Anda merasa perlu, gunakan terminal."
 ],
 "Copy whole directory: $0?": [
  null,
  "Salin seluruh direktori: $0?"
 ],
 "Conflicts found while uploading. Replace?": [
  null,
  "Ditemukan konflik saat mengunggah. Ganti?"
 ],
 "File name can't contain `/`.": [
  null,
  "Nama berkas tidak boleh mengandung `/`."
 ],
 "If you want to move the file, right click > cut then right click > paste.": [
  null,
  "Jika Anda ingin memindahkan berkas, klik kanan > potong lalu klik kanan > tempel."
 ],
 "File name can't be `..`.": [
  null,
  "Nama berkas tidak boleh `..`."
 ],
 "Failed to rename.": [
  null,
  "Gagal mengganti nama."
 ],
 "File exists: ": [
  null,
  "Berkas sudah ada: "
 ],
 "Mode": [
  null,
  "Mode"
 ],
 "Owner": [
  null,
  "Pemilik"
 ],
 "Group": [
  null,
  "Grup"
 ],
 "Size": [
  null,
  "Ukuran"
 ],
 "Accessed": [
  null,
  "Terakhir diakses"
 ],
 "Modified": [
  null,
  "Diubah"
 ],
 "Created": [
  null,
  "Dibuat"
 ],
 "'$0' is not a text file. Open it anyway?": [
  null,
  "'$0' bukan berkas teks. Tetap buka?"
 ],
 "WARNING: this may lead to file corruption.": [
  null,
  "PERINGATAN: ini dapat menyebabkan kerusakan berkas."
 ],
 "Editing ": [
  null,
  "Mengedit "
 ],
 "Link Target": [
  null,
  "Target tautan"
 ],
 "Can't open ": [
  null,
  "Tidak dapat membuka "
 ],
 " for editing.": [
  null,
  " untuk diedit."
 ],
 "%d Directory": [
  null,
  "%d Direktori"
 ],
 "%d File": [
  null,
  "%d Berkas"
 ],
 "selected": [
  null,
  "dipilih"
 ],
 "and": [
  null,
  "dan"
 ],
 "Warning: editing $0 can be dangerous.": [
  null,
  "Peringatan: mengedit $0 bisa berbahaya."
 ],
 "Are you sure?": [
  null,
  "Apakah Anda yakin?"
 ],
 "Warning: editing permissions for $0 files.": [
  null,
  "Peringatan: mengedit izin untuk $0 berkas."
 ],
 "Applying edits to:": [
  null,
  "Menerapkan perubahan ke:"
 ],
 "unchanged": [
  null,
  "tidak berubah"
 ],
 "Deleting $0 files.": [
  null,
  "Menghapus $0 berkas."
 ],
 "Deleting `$0`.": [
  null,
  "Menghapus `$0`."
 ],
 "This cannot be undone. Are you sure?": [
  null,
  "Ini tidak dapat dibatalkan. Apakah Anda yakin?"
 ],
 "Creating Directory": [
  null,
  "Membuat Direktori"
 ],
 "Name: ": [
  null,
  "Nama: "
 ],
 "Directory name can't be empty.": [
  null,
  "Nama direktori tidak boleh kosong."
 ],
 "Directory name can't contain `/`.": [
  null,
  "Nama direktori tidak boleh mengandung `/`."
 ],
 "Creating File": [
  null,
  "Membuat Berkas"
 ],
 "File name can't be empty.": [
  null,
  "Nama berkas tidak boleh kosong."
 ],
 "Creating Symbolic Link": [
  null,
  "Membuat Tautan Simbolik"
 ],
 "Target: ": [
  null,
  "Target: "
 ],
 "Link target can't be empty.": [
  null,
  "Target tautan tidak boleh kosong."
 ],
 "Link name can't be empty.": [
  null,
  "Nama tautan tidak boleh kosong."
 ],
 "Link name can't contain `/`.": [
  null,
  "Nama tautan tidak boleh mengandung `/`."
 ],
 "Overwrite?": [
  null,
  "Timpa?"
 ],
 "Paste failed.": [
  null,
  "Gagal menempel."
 ],
 "Cannot $0 system-critical paths.": [
  null,
  "Tidak dapat $0 jalur kritis sistem."
 ],
 "The following path(s) are very dangerous to $0: $1. If you think you need to, use the terminal.": [
  null,
  "Jalur berikut sangat berbahaya untuk $0: $1. Jika Anda merasa perlu, gunakan terminal."
 ],
 "Cockpit Navigator $0": [
  null,
  "Cockpit Navigator $0"
 ],
 "Created by $0 for Houston UI (Cockpit).": [
  null,
  "Dibuat oleh $0 untuk Houston UI (Cockpit)."
 ],
 "Issue Tracker": [
  null,
  "Pelacak Masalah"
 ],
 "Feedback": [
  null,
  "Umpan Balik"
 ],
 "Back": [
  null,
  "Kembali"
 ],
 "Forward": [
  null,
  "Maju"
 ],
 "Up": [
  null,
  "Atas"
 ],
 "Refresh": [
  null,
  "Segarkan"
 ],
 "Navigation Bar": [
  null,
  "Bilah Navigasi"
 ],
 "New Directory": [
  null,
  "Direktori Baru"
 ],
 "New File": [
  null,
  "Berkas Baru"
 ],
 "New Symbolic Link": [
  null,
  "Tautan Simbolik Baru"
 ],
 "Upload File(s)": [
  null,
  "Unggah Berkas"
 ],
 "Save": [
  null,
  "Simpan"
 ],
 "Delete": [
  null,
  "Hapus"
 ],
 "Edit Properties": [
  null,
  "Edit Properti"
 ],
 "Edit Contents": [
  null,
  "Edit Isi"
 ],
 "Save Changes": [
  null,
  "Simpan Perubahan"
 ],
 "Prepend * to fuzzy search": [
  null,
  "Tambahkan * untuk pencarian kabur"
 ],
 "Search in Directory": [
  null,
  "Cari di Direktori"
 ],
 "Name": [
  null,
  "Nama"
 ],
 "Read": [
  null,
  "Baca"
 ],
 "Write": [
  null,
  "Tulis"
 ],
 "Execute": [
  null,
  "Eksekusi"
 ],
 "Other": [
  null,
  "Lainnya"
 ],
 "Show Hidden Files": [
  null,
  "Tampilkan Berkas Tersembunyi"
 ],
 "Toggle Dark/Light": [
  null,
  "Beralih Gelap/Terang"
 ],
 "Navigator": [
  null,
  "Navigator"
 ]
});
