cockpit.locale({
 "": {
  "plural-forms": (n) => (n > 1),
  "language": "tr",
  "language-direction": "ltr"
 },
 "Uploading ": [
  null,
  "Y\u00fckleniyor "
 ],
 "Failed to read file: ": [
  null,
  "Dosya okunamad\u0131: "
 ],
 "Upload of directories not supported.": [
  null,
  "Dizin y\u00fckleme desteklenmiyor."
 ],
 "Upload of directories and empty files not supported.": [
  null,
  "Dizin ve bo\u015f dosya y\u00fckleme desteklenmiyor."
 ],
 "OK": [
  null,
  "Tamam"
 ],
 "Cancel": [
  null,
  "\u0130ptal"
 ],
 "Yes": [
  null,
  "Evet"
 ],
 "No": [
  null,
  "Hay\u0131r"
 ],
 "Cannot rename system-critical paths.": [
  null,
  "Sistem a\u00e7\u0131s\u0131ndan kritik yollar yeniden adland\u0131r\u0131lamaz."
 ],
 "If you think you need to, use the terminal.": [
  null,
  "Gerekli oldu\u011funu d\u00fc\u015f\u00fcn\u00fcyorsan\u0131z terminali kullan\u0131n."
 ],
 "Copy whole directory: $0?": [
  null,
  "T\u00fcm dizin kopyalans\u0131n m\u0131: $0?"
 ],
 "Conflicts found while uploading. Replace?": [
  null,
  "Y\u00fckleme s\u0131ras\u0131nda \u00e7ak\u0131\u015fmalar bulundu. De\u011fi\u015ftirilsin mi?"
 ],
 "File name can't contain `/`.": [
  null,
  "Dosya ad\u0131 `/` i\u00e7eremez."
 ],
 "If you want to move the file, right click > cut then right click > paste.": [
  null,
  "Dosyay\u0131 ta\u015f\u0131mak istiyorsan\u0131z, sa\u011f t\u0131klay\u0131n > kes, ard\u0131ndan sa\u011f t\u0131klay\u0131n > yap\u0131\u015ft\u0131r."
 ],
 "File name can't be `..`.": [
  null,
  "Dosya ad\u0131 `..` olamaz."
 ],
 "Failed to rename.": [
  null,
  "Yeniden adland\u0131rma ba\u015far\u0131s\u0131z."
 ],
 "File exists: ": [
  null,
  "Dosya mevcut: "
 ],
 "Mode": [
  null,
  "Mod"
 ],
 "Owner": [
  null,
  "Sahip"
 ],
 "Group": [
  null,
  "Grup"
 ],
 "Size": [
  null,
  "Boyut"
 ],
 "Accessed": [
  null,
  "Son eri\u015fim"
 ],
 "Modified": [
  null,
  "De\u011fi\u015ftirilme"
 ],
 "Created": [
  null,
  "Olu\u015fturulma"
 ],
 "'$0' is not a text file. Open it anyway?": [
  null,
  "'$0' bir metin dosyas\u0131 de\u011fil. Yine de a\u00e7\u0131ls\u0131n m\u0131?"
 ],
 "WARNING: this may lead to file corruption.": [
  null,
  "UYARI: bu dosya bozulmas\u0131na yol a\u00e7abilir."
 ],
 "Editing ": [
  null,
  "D\u00fczenleniyor "
 ],
 "Link Target": [
  null,
  "Ba\u011flant\u0131 hedefi"
 ],
 "Can't open ": [
  null,
  "A\u00e7\u0131lam\u0131yor "
 ],
 " for editing.": [
  null,
  " d\u00fczenleme i\u00e7in."
 ],
 "%d Directory": [
  null,
  "%d Dizin",
  "%d Dizin"
 ],
 "%d File": [
  null,
  "%d Dosya",
  "%d Dosya"
 ],
 "selected": [
  null,
  "se\u00e7ili"
 ],
 "and": [
  null,
  "ve"
 ],
 "Warning: editing $0 can be dangerous.": [
  null,
  "Uyar\u0131: $0 d\u00fczenlemek tehlikeli olabilir."
 ],
 "Are you sure?": [
  null,
  "Emin misiniz?"
 ],
 "Warning: editing permissions for $0 files.": [
  null,
  "Uyar\u0131: $0 dosyan\u0131n izinleri d\u00fczenleniyor."
 ],
 "Applying edits to:": [
  null,
  "De\u011fi\u015fiklikler uygulan\u0131yor:"
 ],
 "unchanged": [
  null,
  "de\u011fi\u015ftirilmedi"
 ],
 "Deleting $0 files.": [
  null,
  "$0 dosya siliniyor."
 ],
 "Deleting `$0`.": [
  null,
  "`$0` siliniyor."
 ],
 "This cannot be undone. Are you sure?": [
  null,
  "Bu geri al\u0131namaz. Emin misiniz?"
 ],
 "Creating Directory": [
  null,
  "Dizin Olu\u015fturma"
 ],
 "Name: ": [
  null,
  "Ad: "
 ],
 "Directory name can't be empty.": [
  null,
  "Dizin ad\u0131 bo\u015f olamaz."
 ],
 "Directory name can't contain `/`.": [
  null,
  "Dizin ad\u0131 `/` i\u00e7eremez."
 ],
 "Creating File": [
  null,
  "Dosya Olu\u015fturma"
 ],
 "File name can't be empty.": [
  null,
  "Dosya ad\u0131 bo\u015f olamaz."
 ],
 "Creating Symbolic Link": [
  null,
  "Sembolik Ba\u011flant\u0131 Olu\u015fturma"
 ],
 "Target: ": [
  null,
  "Hedef: "
 ],
 "Link target can't be empty.": [
  null,
  "Ba\u011flant\u0131 hedefi bo\u015f olamaz."
 ],
 "Link name can't be empty.": [
  null,
  "Ba\u011flant\u0131 ad\u0131 bo\u015f olamaz."
 ],
 "Link name can't contain `/`.": [
  null,
  "Ba\u011flant\u0131 ad\u0131 `/` i\u00e7eremez."
 ],
 "Overwrite?": [
  null,
  "\u00dczerine yaz\u0131ls\u0131n m\u0131?"
 ],
 "Paste failed.": [
  null,
  "Yap\u0131\u015ft\u0131rma ba\u015far\u0131s\u0131z."
 ],
 "Cannot $0 system-critical paths.": [
  null,
  "Sistem a\u00e7\u0131s\u0131ndan kritik yollar $0 yap\u0131lamaz."
 ],
 "The following path(s) are very dangerous to $0: $1. If you think you need to, use the terminal.": [
  null,
  "\u015eu yollar $0 i\u00e7in \u00e7ok tehlikelidir: $1. Gerekli oldu\u011funu d\u00fc\u015f\u00fcn\u00fcyorsan\u0131z terminali kullan\u0131n."
 ],
 "Cockpit Navigator $0": [
  null,
  "Cockpit Navigator $0"
 ],
 "Created by $0 for Houston UI (Cockpit).": [
  null,
  "$0 taraf\u0131ndan Houston UI (Cockpit) i\u00e7in olu\u015fturuldu."
 ],
 "Issue Tracker": [
  null,
  "Sorun Takibi"
 ],
 "Feedback": [
  null,
  "Geri Bildirim"
 ],
 "Back": [
  null,
  "Geri"
 ],
 "Forward": [
  null,
  "\u0130leri"
 ],
 "Up": [
  null,
  "Yukar\u0131"
 ],
 "Refresh": [
  null,
  "Yenile"
 ],
 "Navigation Bar": [
  null,
  "Gezinti \u00c7ubu\u011fu"
 ],
 "New Directory": [
  null,
  "Yeni Dizin"
 ],
 "New File": [
  null,
  "Yeni Dosya"
 ],
 "New Symbolic Link": [
  null,
  "Yeni Sembolik Ba\u011flant\u0131"
 ],
 "Upload File(s)": [
  null,
  "Dosya Y\u00fckle"
 ],
 "Save": [
  null,
  "Kaydet"
 ],
 "Delete": [
  null,
  "Sil"
 ],
 "Edit Properties": [
  null,
  "\u00d6zellikleri D\u00fczenle"
 ],
 "Edit Contents": [
  null,
  "\u0130\u00e7eri\u011fi D\u00fczenle"
 ],
 "Save Changes": [
  null,
  "De\u011fi\u015fiklikleri Kaydet"
 ],
 "Prepend * to fuzzy search": [
  null,
  "Bulan\u0131k arama i\u00e7in * ekleyin"
 ],
 "Search in Directory": [
  null,
  "Dizinde Ara"
 ],
 "Name": [
  null,
  "Ad"
 ],
 "Read": [
  null,
  "Okuma"
 ],
 "Write": [
  null,
  "Yazma"
 ],
 "Execute": [
  null,
  "\u00c7al\u0131\u015ft\u0131rma"
 ],
 "Other": [
  null,
  "Di\u011fer"
 ],
 "Show Hidden Files": [
  null,
  "Gizli Dosyalar\u0131 G\u00f6ster"
 ],
 "Toggle Dark/Light": [
  null,
  "Koyu/A\u00e7\u0131k Ge\u00e7i\u015f"
 ],
 "Navigator": [
  null,
  "Gezgin"
 ]
});
