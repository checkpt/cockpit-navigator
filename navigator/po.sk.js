cockpit.locale({
 "": {
  "plural-forms": (n) => (n != 1),
  "language": "sk",
  "language-direction": "ltr"
 },
 "Uploading ": [
  null,
  "Nahr\u00e1vanie "
 ],
 "Failed to read file: ": [
  null,
  "Nepodarilo sa pre\u010d\u00edta\u0165 s\u00fabor: "
 ],
 "Upload of directories not supported.": [
  null,
  "Nahr\u00e1vanie adres\u00e1rov nie je podporovan\u00e9."
 ],
 "Upload of directories and empty files not supported.": [
  null,
  "Nahr\u00e1vanie adres\u00e1rov a pr\u00e1zdnych s\u00faborov nie je podporovan\u00e9."
 ],
 "OK": [
  null,
  "OK"
 ],
 "Cancel": [
  null,
  "Zru\u0161i\u0165"
 ],
 "Yes": [
  null,
  "\u00c1no"
 ],
 "No": [
  null,
  "Nie"
 ],
 "Cannot rename system-critical paths.": [
  null,
  "Nie je mo\u017en\u00e9 premenova\u0165 syst\u00e9movo kritick\u00e9 cesty."
 ],
 "If you think you need to, use the terminal.": [
  null,
  "Ak si mysl\u00edte, \u017ee to potrebujete, pou\u017eite termin\u00e1l."
 ],
 "Copy whole directory: $0?": [
  null,
  "Skop\u00edrova\u0165 cel\u00fd adres\u00e1r: $0?"
 ],
 "Conflicts found while uploading. Replace?": [
  null,
  "Pri nahr\u00e1van\u00ed boli n\u00e1jden\u00e9 konflikty. Nahradi\u0165?"
 ],
 "File name can't contain `/`.": [
  null,
  "N\u00e1zov s\u00faboru nem\u00f4\u017ee obsahova\u0165 `/`."
 ],
 "If you want to move the file, right click > cut then right click > paste.": [
  null,
  "Ak chcete s\u00fabor presun\u00fa\u0165, kliknite prav\u00fdm tla\u010didlom > vystrihn\u00fa\u0165, potom prav\u00fdm tla\u010didlom > vlo\u017ei\u0165."
 ],
 "File name can't be `..`.": [
  null,
  "N\u00e1zov s\u00faboru nem\u00f4\u017ee by\u0165 `..`."
 ],
 "Failed to rename.": [
  null,
  "Premenovanie sa nepodarilo."
 ],
 "File exists: ": [
  null,
  "S\u00fabor existuje: "
 ],
 "Mode": [
  null,
  "Re\u017eim"
 ],
 "Owner": [
  null,
  "Vlastn\u00edk"
 ],
 "Group": [
  null,
  "Skupina"
 ],
 "Size": [
  null,
  "Ve\u013ekos\u0165"
 ],
 "Accessed": [
  null,
  "Posledn\u00fd pr\u00edstup"
 ],
 "Modified": [
  null,
  "Zmenen\u00e9"
 ],
 "Created": [
  null,
  "Vytvoren\u00e9"
 ],
 "'$0' is not a text file. Open it anyway?": [
  null,
  "'$0' nie je textov\u00fd s\u00fabor. Napriek tomu otvori\u0165?"
 ],
 "WARNING: this may lead to file corruption.": [
  null,
  "VAROVANIE: m\u00f4\u017ee to vies\u0165 k po\u0161kodeniu s\u00faboru."
 ],
 "Editing ": [
  null,
  "\u00daprava "
 ],
 "Link Target": [
  null,
  "Cie\u013e odkazu"
 ],
 "Can't open ": [
  null,
  "Nie je mo\u017en\u00e9 otvori\u0165 "
 ],
 " for editing.": [
  null,
  " na \u00fapravu."
 ],
 "%d Directory": [
  null,
  "%d adres\u00e1r",
  "%d adres\u00e1re",
  "%d adres\u00e1rov"
 ],
 "%d File": [
  null,
  "%d s\u00fabor",
  "%d s\u00fabory",
  "%d s\u00faborov"
 ],
 "selected": [
  null,
  "vybrat\u00e9"
 ],
 "and": [
  null,
  "a"
 ],
 "Warning: editing $0 can be dangerous.": [
  null,
  "Varovanie: \u00faprava $0 m\u00f4\u017ee by\u0165 nebezpe\u010dn\u00e1."
 ],
 "Are you sure?": [
  null,
  "Ste si ist\u00ed?"
 ],
 "Warning: editing permissions for $0 files.": [
  null,
  "Varovanie: \u00faprava opr\u00e1vnen\u00ed pre $0 s\u00faborov."
 ],
 "Applying edits to:": [
  null,
  "Pou\u017eitie \u00faprav na:"
 ],
 "unchanged": [
  null,
  "nezmenen\u00e9"
 ],
 "Deleting $0 files.": [
  null,
  "Mazanie $0 s\u00faborov."
 ],
 "Deleting `$0`.": [
  null,
  "Mazanie `$0`."
 ],
 "This cannot be undone. Are you sure?": [
  null,
  "Toto sa ned\u00e1 vr\u00e1ti\u0165 sp\u00e4\u0165. Ste si ist\u00ed?"
 ],
 "Creating Directory": [
  null,
  "Vytvorenie adres\u00e1ra"
 ],
 "Name: ": [
  null,
  "N\u00e1zov: "
 ],
 "Directory name can't be empty.": [
  null,
  "N\u00e1zov adres\u00e1ra nem\u00f4\u017ee by\u0165 pr\u00e1zdny."
 ],
 "Directory name can't contain `/`.": [
  null,
  "N\u00e1zov adres\u00e1ra nem\u00f4\u017ee obsahova\u0165 `/`."
 ],
 "Creating File": [
  null,
  "Vytvorenie s\u00faboru"
 ],
 "File name can't be empty.": [
  null,
  "N\u00e1zov s\u00faboru nem\u00f4\u017ee by\u0165 pr\u00e1zdny."
 ],
 "Creating Symbolic Link": [
  null,
  "Vytvorenie symbolick\u00e9ho odkazu"
 ],
 "Target: ": [
  null,
  "Cie\u013e: "
 ],
 "Link target can't be empty.": [
  null,
  "Cie\u013e odkazu nem\u00f4\u017ee by\u0165 pr\u00e1zdny."
 ],
 "Link name can't be empty.": [
  null,
  "N\u00e1zov odkazu nem\u00f4\u017ee by\u0165 pr\u00e1zdny."
 ],
 "Link name can't contain `/`.": [
  null,
  "N\u00e1zov odkazu nem\u00f4\u017ee obsahova\u0165 `/`."
 ],
 "Overwrite?": [
  null,
  "Prep\u00edsa\u0165?"
 ],
 "Paste failed.": [
  null,
  "Vlo\u017eenie sa nepodarilo."
 ],
 "Cannot $0 system-critical paths.": [
  null,
  "Nie je mo\u017en\u00e9 $0 syst\u00e9movo kritick\u00e9 cesty."
 ],
 "The following path(s) are very dangerous to $0: $1. If you think you need to, use the terminal.": [
  null,
  "Nasleduj\u00face cesty s\u00fa ve\u013emi nebezpe\u010dn\u00e9 pre $0: $1. Ak si mysl\u00edte, \u017ee to potrebujete, pou\u017eite termin\u00e1l."
 ],
 "Cockpit Navigator $0": [
  null,
  "Cockpit Navigator $0"
 ],
 "Created by $0 for Houston UI (Cockpit).": [
  null,
  "Vytvoril $0 pre Houston UI (Cockpit)."
 ],
 "Issue Tracker": [
  null,
  "Sledovanie ch\u00fdb"
 ],
 "Feedback": [
  null,
  "Sp\u00e4tn\u00e1 v\u00e4zba"
 ],
 "Back": [
  null,
  "Sp\u00e4\u0165"
 ],
 "Forward": [
  null,
  "Vpred"
 ],
 "Up": [
  null,
  "Hore"
 ],
 "Refresh": [
  null,
  "Obnovi\u0165"
 ],
 "Navigation Bar": [
  null,
  "Naviga\u010dn\u00fd panel"
 ],
 "New Directory": [
  null,
  "Nov\u00fd adres\u00e1r"
 ],
 "New File": [
  null,
  "Nov\u00fd s\u00fabor"
 ],
 "New Symbolic Link": [
  null,
  "Nov\u00fd symbolick\u00fd odkaz"
 ],
 "Upload File(s)": [
  null,
  "Nahra\u0165 s\u00fabory"
 ],
 "Save": [
  null,
  "Ulo\u017ei\u0165"
 ],
 "Delete": [
  null,
  "Zmaza\u0165"
 ],
 "Edit Properties": [
  null,
  "Upravi\u0165 vlastnosti"
 ],
 "Edit Contents": [
  null,
  "Upravi\u0165 obsah"
 ],
 "Save Changes": [
  null,
  "Ulo\u017ei\u0165 zmeny"
 ],
 "Prepend * to fuzzy search": [
  null,
  "Pridajte * pre pribli\u017en\u00e9 h\u013eadanie"
 ],
 "Search in Directory": [
  null,
  "H\u013eada\u0165 v adres\u00e1ri"
 ],
 "Name": [
  null,
  "N\u00e1zov"
 ],
 "Read": [
  null,
  "\u010c\u00edtanie"
 ],
 "Write": [
  null,
  "Z\u00e1pis"
 ],
 "Execute": [
  null,
  "Sp\u00fa\u0161\u0165anie"
 ],
 "Other": [
  null,
  "Ostatn\u00e9"
 ],
 "Show Hidden Files": [
  null,
  "Zobrazi\u0165 skryt\u00e9 s\u00fabory"
 ],
 "Toggle Dark/Light": [
  null,
  "Prepn\u00fa\u0165 tmav\u00fd/svetl\u00fd re\u017eim"
 ],
 "Navigator": [
  null,
  "Navig\u00e1tor"
 ]
});
