cockpit.locale({
 "": {
  "plural-forms": (n) => (n != 1),
  "language": "es",
  "language-direction": "ltr"
 },
 "Uploading ": [
  null,
  "Subiendo "
 ],
 "Failed to read file: ": [
  null,
  "Error al leer el archivo: "
 ],
 "Upload of directories not supported.": [
  null,
  "La subida de directorios no est\u00e1 soportada."
 ],
 "Upload of directories and empty files not supported.": [
  null,
  "La subida de directorios y archivos vac\u00edos no est\u00e1 soportada."
 ],
 "OK": [
  null,
  "Aceptar"
 ],
 "Cancel": [
  null,
  "Cancelar"
 ],
 "Yes": [
  null,
  "S\u00ed"
 ],
 "No": [
  null,
  "No"
 ],
 "Cannot rename system-critical paths.": [
  null,
  "No se pueden renombrar rutas cr\u00edticas del sistema."
 ],
 "If you think you need to, use the terminal.": [
  null,
  "Si cree que necesita hacerlo, use la terminal."
 ],
 "Copy whole directory: $0?": [
  null,
  "\u00bfCopiar el directorio completo: $0?"
 ],
 "Conflicts found while uploading. Replace?": [
  null,
  "Se encontraron conflictos durante la subida. \u00bfReemplazar?"
 ],
 "File name can't contain `/`.": [
  null,
  "El nombre del archivo no puede contener `/`."
 ],
 "If you want to move the file, right click > cut then right click > paste.": [
  null,
  "Si desea mover el archivo, clic derecho > cortar, luego clic derecho > pegar."
 ],
 "File name can't be `..`.": [
  null,
  "El nombre del archivo no puede ser `..`."
 ],
 "Failed to rename.": [
  null,
  "Error al renombrar."
 ],
 "File exists: ": [
  null,
  "El archivo existe: "
 ],
 "Mode": [
  null,
  "Modo"
 ],
 "Owner": [
  null,
  "Propietario"
 ],
 "Group": [
  null,
  "Grupo"
 ],
 "Size": [
  null,
  "Tama\u00f1o"
 ],
 "Accessed": [
  null,
  "\u00daltimo acceso"
 ],
 "Modified": [
  null,
  "Modificado"
 ],
 "Created": [
  null,
  "Creado"
 ],
 "'$0' is not a text file. Open it anyway?": [
  null,
  "'$0' no es un archivo de texto. \u00bfAbrirlo de todos modos?"
 ],
 "WARNING: this may lead to file corruption.": [
  null,
  "ADVERTENCIA: esto puede provocar corrupci\u00f3n del archivo."
 ],
 "Editing ": [
  null,
  "Editando "
 ],
 "Link Target": [
  null,
  "Destino del enlace"
 ],
 "Can't open ": [
  null,
  "No se puede abrir "
 ],
 " for editing.": [
  null,
  " para editar."
 ],
 "%d Directory": [
  null,
  "%d Directorio",
  "%d Directorios"
 ],
 "%d File": [
  null,
  "%d Archivo",
  "%d Archivos"
 ],
 "selected": [
  null,
  "seleccionados"
 ],
 "and": [
  null,
  "y"
 ],
 "Warning: editing $0 can be dangerous.": [
  null,
  "Advertencia: editar $0 puede ser peligroso."
 ],
 "Are you sure?": [
  null,
  "\u00bfEst\u00e1 seguro?"
 ],
 "Warning: editing permissions for $0 files.": [
  null,
  "Advertencia: editando permisos para $0 archivos."
 ],
 "Applying edits to:": [
  null,
  "Aplicando cambios a:"
 ],
 "unchanged": [
  null,
  "sin cambios"
 ],
 "Deleting $0 files.": [
  null,
  "Eliminando $0 archivos."
 ],
 "Deleting `$0`.": [
  null,
  "Eliminando `$0`."
 ],
 "This cannot be undone. Are you sure?": [
  null,
  "Esto no se puede deshacer. \u00bfEst\u00e1 seguro?"
 ],
 "Creating Directory": [
  null,
  "Creando directorio"
 ],
 "Name: ": [
  null,
  "Nombre: "
 ],
 "Directory name can't be empty.": [
  null,
  "El nombre del directorio no puede estar vac\u00edo."
 ],
 "Directory name can't contain `/`.": [
  null,
  "El nombre del directorio no puede contener `/`."
 ],
 "Creating File": [
  null,
  "Creando archivo"
 ],
 "File name can't be empty.": [
  null,
  "El nombre del archivo no puede estar vac\u00edo."
 ],
 "Creating Symbolic Link": [
  null,
  "Creando enlace simb\u00f3lico"
 ],
 "Target: ": [
  null,
  "Destino: "
 ],
 "Link target can't be empty.": [
  null,
  "El destino del enlace no puede estar vac\u00edo."
 ],
 "Link name can't be empty.": [
  null,
  "El nombre del enlace no puede estar vac\u00edo."
 ],
 "Link name can't contain `/`.": [
  null,
  "El nombre del enlace no puede contener `/`."
 ],
 "Overwrite?": [
  null,
  "\u00bfSobrescribir?"
 ],
 "Paste failed.": [
  null,
  "Error al pegar."
 ],
 "Cannot $0 system-critical paths.": [
  null,
  "No se pueden $0 rutas cr\u00edticas del sistema."
 ],
 "The following path(s) are very dangerous to $0: $1. If you think you need to, use the terminal.": [
  null,
  "Las siguientes rutas son muy peligrosas para $0: $1. Si cree que necesita hacerlo, use la terminal."
 ],
 "Cockpit Navigator $0": [
  null,
  "Cockpit Navigator $0"
 ],
 "Created by $0 for Houston UI (Cockpit).": [
  null,
  "Creado por $0 para Houston UI (Cockpit)."
 ],
 "Issue Tracker": [
  null,
  "Seguimiento de problemas"
 ],
 "Feedback": [
  null,
  "Comentarios"
 ],
 "Back": [
  null,
  "Atr\u00e1s"
 ],
 "Forward": [
  null,
  "Adelante"
 ],
 "Up": [
  null,
  "Arriba"
 ],
 "Refresh": [
  null,
  "Actualizar"
 ],
 "Navigation Bar": [
  null,
  "Barra de navegaci\u00f3n"
 ],
 "New Directory": [
  null,
  "Nuevo directorio"
 ],
 "New File": [
  null,
  "Nuevo archivo"
 ],
 "New Symbolic Link": [
  null,
  "Nuevo enlace simb\u00f3lico"
 ],
 "Upload File(s)": [
  null,
  "Subir archivo(s)"
 ],
 "Save": [
  null,
  "Guardar"
 ],
 "Delete": [
  null,
  "Eliminar"
 ],
 "Edit Properties": [
  null,
  "Editar propiedades"
 ],
 "Edit Contents": [
  null,
  "Editar contenido"
 ],
 "Save Changes": [
  null,
  "Guardar cambios"
 ],
 "Prepend * to fuzzy search": [
  null,
  "Agregar * para b\u00fasqueda aproximada"
 ],
 "Search in Directory": [
  null,
  "Buscar en directorio"
 ],
 "Name": [
  null,
  "Nombre"
 ],
 "Read": [
  null,
  "Lectura"
 ],
 "Write": [
  null,
  "Escritura"
 ],
 "Execute": [
  null,
  "Ejecuci\u00f3n"
 ],
 "Other": [
  null,
  "Otros"
 ],
 "Show Hidden Files": [
  null,
  "Mostrar archivos ocultos"
 ],
 "Toggle Dark/Light": [
  null,
  "Cambiar tema Oscuro/Claro"
 ],
 "Navigator": [
  null,
  "Navigator"
 ]
});
