cockpit.locale({
 "": {
  "plural-forms": (n) => (n != 1),
  "language": "it",
  "language-direction": "ltr"
 },
 "Uploading ": [
  null,
  "Caricamento "
 ],
 "Failed to read file: ": [
  null,
  "Impossibile leggere il file: "
 ],
 "Upload of directories not supported.": [
  null,
  "Caricamento delle cartelle non supportato."
 ],
 "Upload of directories and empty files not supported.": [
  null,
  "Caricamento di cartelle e file vuoti non supportato."
 ],
 "OK": [
  null,
  "OK"
 ],
 "Cancel": [
  null,
  "Annulla"
 ],
 "Yes": [
  null,
  "S\u00ec"
 ],
 "No": [
  null,
  "No"
 ],
 "Cannot rename system-critical paths.": [
  null,
  "Impossibile rinominare percorsi critici di sistema."
 ],
 "If you think you need to, use the terminal.": [
  null,
  "Se pensi di doverlo fare, usa il terminale."
 ],
 "Copy whole directory: $0?": [
  null,
  "Copiare l'intera cartella: $0?"
 ],
 "Conflicts found while uploading. Replace?": [
  null,
  "Conflitti trovati durante il caricamento. Sostituire?"
 ],
 "File name can't contain `/`.": [
  null,
  "Il nome del file non pu\u00f2 contenere `/`."
 ],
 "If you want to move the file, right click > cut then right click > paste.": [
  null,
  "Se vuoi spostare il file, clic destro > taglia poi clic destro > incolla."
 ],
 "File name can't be `..`.": [
  null,
  "Il nome del file non pu\u00f2 essere `..`."
 ],
 "Failed to rename.": [
  null,
  "Impossibile rinominare."
 ],
 "File exists: ": [
  null,
  "Il file esiste: "
 ],
 "Mode": [
  null,
  "Modalit\u00e0"
 ],
 "Owner": [
  null,
  "Proprietario"
 ],
 "Group": [
  null,
  "Gruppo"
 ],
 "Size": [
  null,
  "Dimensione"
 ],
 "Accessed": [
  null,
  "Ultimo accesso"
 ],
 "Modified": [
  null,
  "Modificato"
 ],
 "Created": [
  null,
  "Creato"
 ],
 "'$0' is not a text file. Open it anyway?": [
  null,
  "'$0' non \u00e8 un file di testo. Aprirlo comunque?"
 ],
 "WARNING: this may lead to file corruption.": [
  null,
  "ATTENZIONE: questo potrebbe corrompere il file."
 ],
 "Editing ": [
  null,
  "Modifica di "
 ],
 "Link Target": [
  null,
  "Destinazione collegamento"
 ],
 "Can't open ": [
  null,
  "Impossibile aprire "
 ],
 " for editing.": [
  null,
  " per la modifica."
 ],
 "%d Directory": [
  null,
  "%d Cartella",
  "%d Cartelle"
 ],
 "%d File": [
  null,
  "%d File",
  "%d File"
 ],
 "selected": [
  null,
  "selezionati"
 ],
 "and": [
  null,
  "e"
 ],
 "Warning: editing $0 can be dangerous.": [
  null,
  "Attenzione: modificare $0 pu\u00f2 essere pericoloso."
 ],
 "Are you sure?": [
  null,
  "Sei sicuro?"
 ],
 "Warning: editing permissions for $0 files.": [
  null,
  "Attenzione: modifica dei permessi per $0 file."
 ],
 "Applying edits to:": [
  null,
  "Applicazione delle modifiche a:"
 ],
 "unchanged": [
  null,
  "invariato"
 ],
 "Deleting $0 files.": [
  null,
  "Eliminazione di $0 file."
 ],
 "Deleting `$0`.": [
  null,
  "Eliminazione di `$0`."
 ],
 "This cannot be undone. Are you sure?": [
  null,
  "Questa operazione non pu\u00f2 essere annullata. Sei sicuro?"
 ],
 "Creating Directory": [
  null,
  "Creazione cartella"
 ],
 "Name: ": [
  null,
  "Nome: "
 ],
 "Directory name can't be empty.": [
  null,
  "Il nome della cartella non pu\u00f2 essere vuoto."
 ],
 "Directory name can't contain `/`.": [
  null,
  "Il nome della cartella non pu\u00f2 contenere `/`."
 ],
 "Creating File": [
  null,
  "Creazione file"
 ],
 "File name can't be empty.": [
  null,
  "Il nome del file non pu\u00f2 essere vuoto."
 ],
 "Creating Symbolic Link": [
  null,
  "Creazione collegamento simbolico"
 ],
 "Target: ": [
  null,
  "Destinazione: "
 ],
 "Link target can't be empty.": [
  null,
  "La destinazione del collegamento non pu\u00f2 essere vuota."
 ],
 "Link name can't be empty.": [
  null,
  "Il nome del collegamento non pu\u00f2 essere vuoto."
 ],
 "Link name can't contain `/`.": [
  null,
  "Il nome del collegamento non pu\u00f2 contenere `/`."
 ],
 "Overwrite?": [
  null,
  "Sovrascrivere?"
 ],
 "Paste failed.": [
  null,
  "Incolla non riuscito."
 ],
 "Cannot $0 system-critical paths.": [
  null,
  "Impossibile $0 percorsi critici di sistema."
 ],
 "The following path(s) are very dangerous to $0: $1. If you think you need to, use the terminal.": [
  null,
  "I seguenti percorsi sono molto pericolosi da $0: $1. Se pensi di doverlo fare, usa il terminale."
 ],
 "Cockpit Navigator $0": [
  null,
  "Cockpit Navigator $0"
 ],
 "Created by $0 for Houston UI (Cockpit).": [
  null,
  "Creato da $0 per Houston UI (Cockpit)."
 ],
 "Issue Tracker": [
  null,
  "Segnalazione problemi"
 ],
 "Feedback": [
  null,
  "Feedback"
 ],
 "Back": [
  null,
  "Indietro"
 ],
 "Forward": [
  null,
  "Avanti"
 ],
 "Up": [
  null,
  "Su"
 ],
 "Refresh": [
  null,
  "Aggiorna"
 ],
 "Navigation Bar": [
  null,
  "Barra di navigazione"
 ],
 "New Directory": [
  null,
  "Nuova cartella"
 ],
 "New File": [
  null,
  "Nuovo file"
 ],
 "New Symbolic Link": [
  null,
  "Nuovo collegamento simbolico"
 ],
 "Upload File(s)": [
  null,
  "Carica file"
 ],
 "Save": [
  null,
  "Salva"
 ],
 "Delete": [
  null,
  "Elimina"
 ],
 "Edit Properties": [
  null,
  "Modifica propriet\u00e0"
 ],
 "Edit Contents": [
  null,
  "Modifica contenuto"
 ],
 "Save Changes": [
  null,
  "Salva modifiche"
 ],
 "Prepend * to fuzzy search": [
  null,
  "Aggiungi * per ricerca approssimativa"
 ],
 "Search in Directory": [
  null,
  "Cerca nella cartella"
 ],
 "Name": [
  null,
  "Nome"
 ],
 "Read": [
  null,
  "Lettura"
 ],
 "Write": [
  null,
  "Scrittura"
 ],
 "Execute": [
  null,
  "Esecuzione"
 ],
 "Other": [
  null,
  "Altro"
 ],
 "Show Hidden Files": [
  null,
  "Mostra file nascosti"
 ],
 "Toggle Dark/Light": [
  null,
  "Cambia tema Scuro/Chiaro"
 ],
 "Navigator": [
  null,
  "Navigatore"
 ]
});
